...
export function buildAnalysisPrompt(
  ...
  return [
    ...
    '  "teacher", "interviewer" when unnamed). Give "name" AS IT SHOULD APPEAR IN THE TARGET LANGUAGE, with the',
    '  source form in parentheses (e.g. "Đường Tam Tạng (唐三藏)" for a Vietnamese target).',
    '- "glossary": array of {"source","target"} for recurring names/terms that must stay consistent across the',
    '  whole video. Include EVERY recurring character/place name with its target-language rendering — the dub',
    '  is SPOKEN, so source-script characters cannot be pronounced (Chinese names → Sino-Vietnamese when the',
    '  target is Vietnamese, e.g. 悟空 → "Ngộ Không"). Technical terms get their standard target translation.',
    pronounAsk,
