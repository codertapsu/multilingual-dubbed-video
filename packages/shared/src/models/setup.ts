 ...
 *
 * Derived ONLY from stable specs (core count + total RAM) — never from free
 * memory, which fluctuates between two visits to Settings and would make the
 * limit appear to flap.
 */
export interface CapacityRecommendation {
  /** Recommended simultaneous projects (>=1). */
  maxProjects: number;
  /** Admission budget: `maxProjects * 2` (a local run costs 2, cloud-only 1). */
  budgetPoints: number;
  /**
   * Runs that need a heavy local engine (llama.cpp, whisper.cpp, LibreTranslate,
   * separation, alignment) that may execute concurrently. ALWAYS 1 on every
   * machine: those engines evict each other by design, so a second concurrent
   * one is a failure, not slowness. Not user-overridable.
   */
  heavyLanes: 1;
  /** Slots the CPU allows (before the RAM/hard-cap minimum). */
  cpuSlots: number;
  /** Slots the RAM allows (before the CPU/hard-cap minimum). */
  ramSlots: number;
...
export interface SystemProfileResponse {
  ...
  /** Hardware-derived simultaneous-run capacity. */
...
export interface UpdatePreferences {
  ...
  /** How many projects may dub at once (see {@link ConcurrencyPreferences}). */
  concurrency?: ConcurrencyPreferences;
}

/** User control over simultaneous dubbing runs. */
export interface ConcurrencyPreferences {
  /**
   * `auto` (default) follows the hardware recommendation — stored as INTENT, so
   * a RAM upgrade or an improved heuristic raises the limit instead of
   * stranding the user on a stale number. `manual` pins {@link maxProjects}.
   */
  mode: 'auto' | 'manual';
  /** The pinned limit when `mode` is `manual` (clamped to 1..8). */
  maxProjects?: number;
  /** When true, nothing new starts; queued projects stay queued. */
  paused?: boolean;
}
...
export interface QueueEntry {
  ...
  position: number;
  reason: QueueReason;
  /** Human-readable explanation, generated by the scheduler that decided it. */
  message: string;
}

/** A currently-executing run (for the "Now dubbing" section). */
export interface RunningEntry {
  projectId: string;
  name: string;
  /** True when this run holds the exclusive heavy-engine lane. */
  heavy: boolean;
}

/** GET /queue — the live scheduler state. */
export interface QueueState {
  /** Effective limit in force (hardware recommendation or the manual pin). */
  maxProjects: number;
  /** Admission budget points in force. */
  budgetPoints: number;
  /** Points currently consumed by running projects. */
  usedPoints: number;
  /** Queue paused by the user. */
  paused: boolean;
  running: RunningEntry[];
  entries: QueueEntry[];
  /** The hardware recommendation, so the UI can explain the limit. */
  capacity: CapacityRecommendation;
}
