...
$busy = Get-NetTCPConnection -State Listen -LocalPort 5100 -ErrorAction SilentlyContinue
if ($busy) {
    Write-Warn 'Something is already listening on :5100. Run .\scripts\stop.ps1 first. Aborting.'
    exit 1
}

# Launch dev.ps1 detached; it manages and cleans up its own children.
...
Write-Host '  ----------------------------------------------------------------'
Write-Host '  Angular UI:   http://localhost:1420'
Write-Host '  Orchestrator: http://127.0.0.1:5100'
